...
def has_setpoint_changed(tstat, setpoint_data, zone, building):
	...
	if flag_changed:
		...
		logfile.write("THERMOSTAT CHANGED MANUALY AT : " + datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S') + " UTC")
		logfile.close()
	return flag_changed
...
class ZoneThread(threading.Thread):
	...
	def run(self):
		...
		while True:
			...
			if action_data is not None and has_setpoint_changed(self.tstats[self.zone], action_data, self.zone, self.building):
...
if __name__ == '__main__':
	...
	for zone, tstat in tstats.items():
		thread = ZoneThread(yaml_filename, tstats, zone, client, zone_thermal_models[zone], sys.argv[1])
		thread.start()
