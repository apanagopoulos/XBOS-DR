...
def read_line(line):
    ...
    string_time, message = line.split(";")
    line_time = utils.get_mdal_string_to_datetime(string_time)
...
def mpc_log(building, zone, current_time, interval, is_mpc, is_schedule, mpc_lambda, expansion):
    ...
    if last_line_main_msg == "Placeholder":
        ...
        if last_line_time < current_time - datetime.timedelta(minutes=interval):
            ...
            messages_to_write.append(create_line(last_line_time, "System Off"))
    ...
    if is_schedule and not last_line_main_msg == "Expansion started":
        messages_to_write.append(create_line(current_time, "Expansion started", "Expansion=%f" % expansion)) # TODO What is expansion. percent or constant number. assuming normal schedule stuff.??
    else:
        # TODO, what is this? should we use elif instead of if?
        print "WARNING: this shouldn't be happening"
...
if __name__ == "__main__":
    ...
    mpc_log(BUILDING, ZONE, utils.get_utc_now(), 2, True, False, 0.95, 0)

    mpc_log(BUILDING, ZONE, utils.get_utc_now(), 2, True, False, 0.70, 0)

    mpc_log(BUILDING, ZONE, utils.get_utc_now(), 2, False, True, 0.70, 10)

    mpc_log(BUILDING, ZONE, utils.get_utc_now(), 2, False, True, 0.70, 20)


    time.sleep(30)
    mpc_log(BUILDING, ZONE, utils.get_utc_now(), 0.25, True, False, 0.70, 0)
