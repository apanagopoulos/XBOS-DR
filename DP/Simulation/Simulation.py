...
class Thermal:
    ...
    def __init__(self, cfg, client, now):
        ...
        thermal_data = utils.get_data(cfg=cfg, client=client, days_back=150, force_reload=False, end = now)
...
class Occupancy:
    ...
    def __init__(self, client, hod_client, cfg, advise_cfgs, zones, now):
        ...
        for zone in self.zones:
            if self.advise_cfgs[zone]["Advise"]["Occupancy_Sensors"] == True:
                ...
                self.occ_array[zone] = self.preprocess_occ_non_sensors(zone, self.now)
    ...
    def preprocess_occ_non_sensors(self, zone, now):

        occupancy_array = self.advise_cfgs[zone]["Advise"]["Occupancy"]

        now_time = now.astimezone(tz=pytz.timezone(self.cfg["Pytz_Timezone"]))
        starting_time = now.astimezone(tz=pytz.timezone(self.cfg["Pytz_Timezone"]))
        occupancy = []

        while now_time <= starting_time + timedelta(hours=self.advise_cfgs[zone]["Advise"]["MPCPredictiveHorizon"]):
            i = now_time.weekday()
    ...
    def update_occ_array(self, new_occs, now):
        ...
        for zone in self.zones:
            if self.advise_cfgs[zone]["Advise"]["Occupancy_Sensors"]:
                ...
                self.occ_array[zone] = self.preprocess_occ_non_sensors(zone, now)

#this is the simulaton class
class Simulation:

    def __init__(self, building, hours=24, cfg=None, plot_friendly_output=True, starting_date = None):
    ...
    def run(self):
        ...
        while now_time < self.now + timedelta(hours=self.hours):
            ...
            self.Occ.update_occ_array(occupancy_dict, now_time)
...
if __name__ == '__main__':
    ...
    #starting date, if given should be utc
    sim = Simulation("ciee", hours=1, cfg=None, plot_friendly_output=False)
    sim.run()